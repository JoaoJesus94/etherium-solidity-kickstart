...
describe("Campaigns", () => {
  ...
  it("processes requests", async () => {
    let previousBalance = await web3.eth.getBalance(accounts[9]);
    previousBalance = web3.utils.fromWei(previousBalance, "ether");
    previousBalance = parseFloat(previousBalance);

    await campaign.methods
      .contribute()
      .send({ value: web3.utils.toWei("10", "ether"), from: accounts[1] });
    ...
    let actualBalance = await web3.eth.getBalance(accounts[9]);
    ...
    assert(actualBalance > previousBalance);
  });
});
